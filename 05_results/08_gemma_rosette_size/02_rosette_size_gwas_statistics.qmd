---
title: "GWAS statistics: rosette size"
author: "Tom Ellis"
date: today
format:
  html:
...
noK_output_paths <- Sys.glob(
  "05_results/08_gemma_rosette_size/output/no_K/*.assoc.txt"
)
...
The following three plots show GWAS for rosette size without a relatedness matrix for the parents and two cohorts of F9s.
...
The plot below shows p-values for GWAS for rosette size without a K matrix, comparing parents to two cohorts of F9 plants. P values are correlated roughly as well (or poorly) between generations, as between F9 cohorts.
...
The plot below shows ordered p-values for the parents against both cohorts of F9s. They're well under the diagonal, and there's a step in the same place.
...
The plot below shows effect sizes for of SNPs estimated by GEMMA without a K matrix for rosette size, comparing parents to two cohorts of F9 plants. These effects sizes are even more strongly correlated than were p-values.
...
withK_output_paths <- Sys.glob(
  "05_results/08_gemma_rosette_size/output/with_K/*.assoc.txt"
)
# Drop the file with all F9s combined.
...
Here are Manhattan plots for GWAS on the same data, but this time including a kinship matrix. The peaks are modest, and anything but consistent between cohorts.
...
Effect-size estimate with and without a kinship matrix. Effects are compressed in the latter.

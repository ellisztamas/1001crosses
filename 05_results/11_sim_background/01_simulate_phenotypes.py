...
outdir='/scratch-cbe/users/thomas.ellis/crosses/05_results/11_sim_background'
# outdir = '05_results/11_sim_background/output/tmp'
...
for i in range(snps_to_simulate.shape[0]):
    ...
    # Set an effect per locus that is positive or negative at random. 
    effect_per_locus = 0.01 * np.random.choice([-1, 1], n_background_snps)

    for cohort in geno.keys():
        # Total background  effects on the logit scale
        logit_background = (effect_per_locus[:, np.newaxis] * background_snps[cohort]).sum(0).data

...
outdir=$scratchdir/09_infer_haplotypes/01_sample_sheet
mkdir -p $outdir
...
imputation_sample_sheet=$outdir/imputation_sample_sheet.ped
...
# Note that the grep is wrong!
# This will also get rows with "incorrect", and with "swap" in other columns
# I will ensure to double check lines later when combining VCF files.
...
# Concatenate the two .ped files
cat $resequenced_ped $lowcoverage_ped > $imputation_sample_sheet

# Check for duplicates.
# Prints to stout
awk -F'\t' '{count[$2]++} END {for (item in count) print item "\t" count[item]}' $imputation_sample_sheet

cp $imputation_sample_sheet 03_processing/09_impute_haplotypes/output/

---
title: "Validation of the ancestry of F8 genotypes"
author: "Tom Ellis"
date: "2024-09-17"
output:
...
Figure \@ref(fig:good-example) shows a good example of how such a genome should look.
...
```{r good-example, fig.cap="An example of an F8 individual which is homozygous for one of the two parents at each window in the genome."}
path_plot("1_A3_plot_ibd.png")

```

Figure \@ref(fig:noisy-data) shows a similar example of an F8 that matches one or both parents in each window.
...
```{r noisy-data, fig.cap="An example of an F8 individual which matches one parent across the genome, but the signal is noisy."}

path_plot("4_F9_plot_ibd.png")
```

## Residual heterozygosity

Figure \@ref(fig:residual-heterozygosity) shows an example of an F8 individual that matches one of the two expected parents, except for a two regions on chromosome 5.
...
```{r residual-heterozygosity, fig.cap="An example of an F8 individual with residual heterozygosity on chromosome 5."}
...
NGS libraries.
Not surprisingly, it was not possible to call SNPs at almost any window, so the plots look nearly empty (figure \@ref(fig:no-data)).
This should be easy to fix by sowing more seeds and repeating the genotyping.

```{r no-data, fig.cap="An example of an F8 individual with residual heterozygosity on chromosome 5."}
path_plot('1_E7_plot_ibd.png')
```

## Expected parents are not in the reference panel

Figure \@ref(fig:parents-not-in-ref) shows a case where nothing matches.
...
Likewise, figure \@ref(fig:one-parent-nir) shows an F8 from a cross between accessions 1137 and 8241.
...
```{r parents-not-in-ref, fig.cap="The parents of this cross were not in the reference panel, so no blue or red lines are plotted at all."}
path_plot('3_D5_plot_ibd.png')
```

```{r one-parent-nir, fig.cap="An example where one parent is missing from the reference panel, but the other is present, and a good match."}
path_plot('1_D8_plot_ibd.png')
```

## Labels were swapped

Sample 2_H4 doesn't match its expected parents 9481 and 8307 (figure \@ref(fig:labels-swapped)).
However, there are grey lines close to zero across the genome, suggesting that 
something else might fit.

```{r labels-swapped, fig.cap="This sample does not match the expected parents, but it does look like it matches something else."}
...
Figure \@ref(fig:self-pollinated) shows what should be a cross between accessions 6108 and 6192.
...
```{r self-pollinated, fig.cap="Example of an F8 plant that is homozygous across the whole genome."}
...
Figure \@ref(fig:missing-haplotypes) shows a sample that matches an expected parent at most of the genome, but there are some regions of chromosomes 1, 3, 4 and 5 that don't match either parent, and don't really match anything else in the reference panel.

```{r missing-haplotypes, fig.cap="This F8 plant matches the expected parent at most of the genome, but it is likely that the parts of the 6198 genome are still segregating."}
...
Sample 1_C4 matches expected parent 8283, but not 9471 (figure \@ref(fig:incorrect-parent)).
...
```{r incorrect-parent, fig.cap="Sample that matches one parent, but the other parent appears to be missing."}
path_plot('1_C4_plot_ibd.png')
```
...
Sample 1_G10 matches expected parent 8256, but not 1367 (figure \@ref(fig:ambiguous-swap)).
...
```{r ambiguous-swap, fig.cap="Sample that matches one but not both parents, and there is another candidate that partially matches the remaining haplotypes."}
...
* 322 lines are good matches to expected parents. *No action required*.
* 42 lines are likely matches to the expected parents, but the parent of the cross does not match the parent in the reference panel (because there is probably residual heterozygosity in the parental seed stock). *Sequence the F8s at higher coverage*.
* 17 samples have no data. *Grow the plants again and repeat sequencing*.
* 2 lines are self-pollinated. *Discard these lines*.
* 8 lines have one or both parents that are likely correct, but the parents (1137 and 1074) are not in the reference panel. *Sequence 1137 and 1074 at high coverage*.
* 6 lines do not match the expected parents, but are a close match to another accession in the reference panel. *Swap the labels*.
* 11 lines do match one or both parents, and there is an ambiguous match to another accession in the reference panel. *Either resequence or discard*.
* 22 lines do not match one or both parents. The true parent is likely not in the reference panel. *Sequence these at higher coverage*.

...
# this pyhton script is taken from Rahul Pisupati
# it takes the SNPmatch output oin JSON files and creates more easily readable CSV files
# Input:
# 1. Output file
...
for ef_ix in range(len(input_ids)):
    #window_data = pd.read_csv(input_files[ef_ix],sep = "\t", dtype = {'acc': str}, names = ['acc', 'matchingSNPs', 'infoSNPs', 'matchProb', 'likelihood', 'identical', 'matchStrains', 'window_index'], header = None)
...
main_output.to_csv(args.output_file + ".csv", index=False)
# sample_ancestry.to_csv(args.output_file + "_ancestry.csv", index=True)
log.info("finished!")
